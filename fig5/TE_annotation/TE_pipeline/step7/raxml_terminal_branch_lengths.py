...
if len(sys.argv)!=2:
	print('Please include the slurm alias of your family of interest as a command line argument. E.g. python3 raxml_terminal_branch_lengths.py 62')
	sys.exit()
...
familyname = TEcopyobjs[fragobjs[branchlist[0][0].split('_')[0]].TEcode].RMname
...
os.chdir("..")
outF = open('raxml_collected.out', 'w')
outF.write('exemplar\tTEcode\tfrag(s)\tbranchlen\tinsertion_time\tisFL\tclassification\n')
for RMname, d in final_branch_lengths.items():
	for fcode, branchlength in d.items():
